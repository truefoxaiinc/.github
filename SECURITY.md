...
We will acknowledge a complete report, investigate it, coordinate remediation and provide disclosure guidance. Response times are targets only until a staffed security contact and service-level policy are published.

## Supported versions

Each repository must identify its supported versions. Unless stated otherwise, only the current default branch is supported.
